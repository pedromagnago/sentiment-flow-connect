-- Phase 1: DB + Storage + RLS for OptiCore onboarding
-- 1) Profiles table
...
create policy if not exists "Users can view onboarding forms for their company or own"
...
create policy if not exists "Users can create onboarding forms for their company"
  on public.formularios_onboarding
  for insert
  to authenticated
  with check (
...
create policy if not exists "Users can update onboarding forms they can view"
  on public.formularios_onboarding
  for update
  to authenticated
  using (
...
create policy if not exists "Users can delete onboarding forms they can view"
  on public.formularios_onboarding
  for delete
  to authenticated
  using (
...
create trigger if not exists update_formularios_onboarding_updated_at
...
create policy if not exists "Users can view documents for their company or own"
...
create policy if not exists "Users can insert documents for their company"
  on public.documentos
  for insert
  to authenticated
  with check (
...
create policy if not exists "Users can update documents they can view"
  on public.documentos
  for update
  to authenticated
  using (
...
create policy if not exists "Users can delete documents they can view"
  on public.documentos
  for delete
  to authenticated
  using (
...
create policy if not exists "Onboarding: users can read own files"
  on storage.objects
  for select
  to authenticated
  using (
...
create policy if not exists "Onboarding: users can upload to own folder"
  on storage.objects
  for insert
  to authenticated
  with check (
...
create policy if not exists "Onboarding: users can update own files"
  on storage.objects
  for update
  to authenticated
  using (
...
create policy if not exists "Onboarding: users can delete own files"
  on storage.objects
  for delete
  to authenticated
  using (
